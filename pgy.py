...
def register_sign(url):
    ...
    jt = execjs.compile(jsStr).call('sign', path)
    return jt
...
def get_mcn(r_url):
    sign = register_sign(r_url)
    headers['x-s'] = str(sign['X-s'])
    headers['x-t'] = str(sign['X-t'])
    resp = requests.get(url=r_url, headers=headers).json()
    print(resp)


if __name__ == '__main__':
    # rs = register_sign("https://pgy.xiaohongshu.com/api/solar/cooperator/mcns?sort=&column=&pageNum=1&pageSize=20")
    page = 1
    pgy = Pgy()
    uni_map = pgy.get_uni_map()
    while page <= 16:
        # get_pgy(page)
        url = f"https://pgy.xiaohongshu.com/api/solar/cooperator/mcns?sort=&column=&pageNum={page}&pageSize=20"
        get_mcn(url)
        page += 1
        time.sleep(4)
